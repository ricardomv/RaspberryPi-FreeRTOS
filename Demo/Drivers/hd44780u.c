 ...
 *
 * Port to RaspberryPi running freeRTOS:
...
// Note timing changes for new version of delayMicroseconds ()
  ...
  bcm2835_gpio_write (lcd->strbPin, 1) ; delayMicroseconds (50) ;
  bcm2835_gpio_write (lcd->strbPin, 0) ; delayMicroseconds (50) ;
}
...
static void sendDataCmd (const struct lcdDataStruct *lcd, unsigned char data)
{
  ...
  if (lcd->bits == 4)
  {
    ...
    for (i = 0 ; i < 4 ; ++i)
    {
      bcm2835_gpio_write (lcd->dataPins [i], (d4 & 1)) ;
      d4 >>= 1 ;
    }
    strobe (lcd) ;
  ...
  else
  {
    for (i = 0 ; i < 8 ; ++i)
    {
      bcm2835_gpio_write (lcd->dataPins [i], (myData & 1)) ;
      myData >>= 1 ;
    }
  }
  strobe (lcd) ;
...
{
  bcm2835_gpio_write (lcd->rsPin,   0) ;
...
static void put4Command (const struct lcdDataStruct *lcd, unsigned char command)
{
  ...
  for (i = 0 ; i < 4 ; ++i)
  {
    bcm2835_gpio_write (lcd->dataPins [i], (myCommand & 1)) ;
    myCommand >>= 1 ;
  }
  strobe (lcd) ;
}
...
void lcdCharDef (const int fd, int index, unsigned char data [8])
{
  ...
  bcm2835_gpio_write (lcd->rsPin, 1) ;
  for (i = 0 ; i < 8 ; ++i)
...
// Create a new LCD:
  ...
  bcm2835_gpio_fsel (lcd->rsPin, OUTPUT) ;
  bcm2835_gpio_write (lcd->rsPin, 0) ;
  bcm2835_gpio_fsel (lcd->strbPin, OUTPUT) ;
  bcm2835_gpio_write (lcd->strbPin, 0) ;

  for (i = 0 ; i < bits ; ++i)
  {
    bcm2835_gpio_write (lcd->dataPins [i], 0) ;
    bcm2835_gpio_fsel (lcd->dataPins [i], OUTPUT) ;
  }
  vTaskDelay (35) ; // mS
...
//	mode which sends the commands as 2 x 4-bit values.
  ...
  if (bits == 4)
  {
    ...
    put4Command (lcd, func >> 4) ; vTaskDelay (35) ;
    put4Command (lcd, func >> 4) ; vTaskDelay (35) ;
    put4Command (lcd, func >> 4) ; vTaskDelay (35) ;
  ...
  else
  {
    ...
    putCommand  (lcd, func     ) ; vTaskDelay (35) ;
    putCommand  (lcd, func     ) ; vTaskDelay (35) ;
    putCommand  (lcd, func     ) ; vTaskDelay (35) ;
  }
